...
<body onload="javascript:document.forms[0].submit()">
  ...
  validateIdToken(tokenSet, nonce, returnedBy, maxAge) {
    ...
    if (payload.iss !== undefined) {
      // NOTE:
      // Microsoft templatizes issuer...
      // May need to make issuer verification configurable...
      const issuer = this.issuer.issuer.replace('{tenantid}', payload.tid);
      assert.equal(issuer, payload.iss, 'unexpected iss value');
    }
